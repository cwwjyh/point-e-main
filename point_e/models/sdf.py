...
class CrossAttentionPointCloudSDFModel(PointCloudSDFModel):
    ...
    def predict_sdf(
        ...
        data = encoded["latents"] #(1, 4096, 256)
        x = self.decoder_input_proj(x.permute(0, 2, 1))
        x = self.decoder(x, data)
        x = self.ln_post(x) #(1, 4096, 256)
        x = self.output_proj(x) #tensor:(1,4096,1)
        return x[..., 0] # （1，4096，1）看在那个文件中可以调用其返回值
