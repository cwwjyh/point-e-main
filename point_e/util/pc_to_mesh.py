...
def marching_cubes_mesh(
    ...
    indices = range(0, grid_size**3, batch_size) #range(0,32768,4096)
    if progress:
        indices = tqdm(indices) #indices:0,4096,8192,12288,16384,20480,24576,28672
    ...
    for i in indices:
        ...
        with torch.no_grad():
            volume.append(model(coords[None], encoded=cond)[0]) #{list:8}0={Tensor:(4096)}返回sdf，即sdf.py中139行
    volume_np = torch.cat(volume).view(grid_size, grid_size, grid_size).cpu().numpy() #{ndarray:(32,32,32)}
